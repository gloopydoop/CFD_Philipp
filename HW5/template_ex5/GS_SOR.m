...
while (r(m) > tol) &&  (m < maxit)
    ...
    % B.C. P_[1,1]
    p(1) = 0;
    %p(1) = (1-omega)*p(1) + omega/(1 + beta^2)*(p(2) + beta^2*p(Nx) - hx^2*f(1));
    ...
    for j = 2:(Ny-1)
        ...
        for i = 2:(Nx-1)
            k = i+Nx*(j-1);
            p(k) = (1-omega)*p(k) + omega/(2*(1 + beta^2))*(p(k-1) + p(k+1) + beta^2*(p(k + Nx) + p(k -Nx)) - hx^2*f(k));
        end
        % B.C. P_[Nx,j]
        k = Nx+Nx*(j-1);
        p(k) = (1-omega)*p(k) + omega/(1 + 2*beta^2)*(p(k-1)  + beta^2*(p(k + Nx) + p(k -Nx)) - hx^2*f(k));
